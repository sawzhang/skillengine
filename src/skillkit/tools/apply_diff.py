"""Utilities for applying patch hunks to text content.

This module is adapted from OpenAI's `apply_diff.py` in openai-agents-python.
It applies V4A-style update/create diff sections to a single file content string.
"""
...
def apply_diff(input_text: str, diff: str, mode: ApplyDiffMode = "default") -> str:
    """Apply a diff section to input text."""
...
def _apply_chunks(input_text: str, chunks: list[Chunk], newline: str) -> str:
    ...
    for chunk in chunks:
        if chunk.orig_index > len(orig_lines):
            raise ValueError(
                f"apply_diff: chunk.orig_index {chunk.orig_index} > input length {len(orig_lines)}"
            )
